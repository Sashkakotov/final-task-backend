...
export const findPoints = async (req: Request, res: Response) => {
  ...
  if (ids) {
    ...
    const allPoints = await pointService.findPoints({});
    return res.json(
      allPoints.filter((onePoint) => boards.includes(onePoint.boardId))
    );
  } else {
...
export const createPoint = async (req: Request, res: Response) => {
  ...
  const { title, taskId, boardId, done, startDate, endDate } = req.body;
  try {
    const newPoint = await pointService.createPoint(
      { title, taskId, boardId, done, startDate, endDate },
      guid,
      initUser
    );
    res.json(newPoint);
...
export const updatePoint = async (req: Request, res: Response) => {
  ...
  const { title, done, startDate, endDate } = req.body;

  try {
    const updatedPoint = await pointService.updatePoint(
      req.params.pointId,
      { title, done, startDate, endDate },
      guid,
      initUser
    );
...
export const updateSetOfPoints = async (req: Request, res: Response) => {
  ...
  for (const onePoint of points) {
    ...
    const { _id, done, startDate, endDate } = onePoint;
  ...
  socket.emit("points", {
    action: "update",
    users: await boardService.getUserIdsByBoardsIds(
      updatedPoints.map((item) => item.boardId)
    ),
...
export const deletePoint = async (req: Request, res: Response) => {
  ...
  const initUser = req.header("initUser") || "undefined";
  try {
    const deletedPoint = await pointService.deletePointById(
      req.params.pointId,
      guid,
      initUser
    );
